...
import CompanyRateReactTable from 'sections/cabprovidor/vendorManagement/vendorOverview/vendorRate/CompanyRateReactTable';
import CompanyRateForm from './CompanyRateForm';
...
const CompanyRates = ({id,companyName}) => {
  ...
  useEffect(() => {
    const fetchdata = async () => {
      ...
      setCompanyRate(response.data.data.result);
    };
  ...
  const handleSaveRate = async () => {
    ...
    const formValues = companyRate.map((item) => {
      // Convert dualTripAmount to object with vehicleTypeID as key
      const dualTripAmountObj = item.dualTripAmount.reduce((acc, curr) => {
        acc[curr.vehicleTypeID] = curr.amount;
        return acc;
      }, {});
    ...
    <>
      <MainCard title="Company Rates" content={false} secondary={<CSVExport data={data} filename={'cell-editable-table.csv'} />}>
        <ScrollX>
          ...
          {(companyRate.length !== 0 || companyList.length !== 0) && (
            <CompanyRateReactTable
              columns={columns}
              data={[...companyRate]} // Combine companyRate and companyList
