export function getMergeResult(data, zone_zoneType, vehicleType) {
    ...
    data.forEach((val) => {
        ...
        if (zoneTypeArray.length > 0) {
            ...
            let matchedZoneTypes = zoneTypeArray.filter(type => 
                type.zoneTypeName.toLowerCase() === obj.zoneType.toLowerCase()
            );
        ...
        obj.status = (zoneNameArray.length === 1 && vehicleTypeArray.length === 1 && obj.zoneTypeArray.length === 1) ? 1 : 0;
